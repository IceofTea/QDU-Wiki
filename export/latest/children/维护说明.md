...
步骤：
1.
把要分享的文件放入 docs/share/files/ 目录
2.
在文件共享主页（docs/share/index.md）的列表中，按 Markdown 格式添加一行引用：
‌
1
[资料名称](files/资料名称.pdf)
‌
...
渲染后即为可点击下载的链接。
注意事项：
文件请用简洁贴切的中文名，如 青岛大学本科新生指南.pdf
体积较大的文件（如几十 MB 的压缩包、视频）不建议直接放进仓库，可先上传到网盘（百度网盘、阿里云盘）或 GitHub Releases，再在主页放下载链接：
‌
1
[资料名称](下载链接)
‌
涉及个人隐私的信息（姓名、学号、身份证号等）请先脱敏再上传
上传图片
步骤：
1.
图片统一放在 docs/pics/ 目录下，并按板块放入对应的子文件夹：
子文件夹
...
‌
2.
命名格式：<页面名>-图<编号>-<说明>.png，例如 study-图1-校历.png
3.
在页面中按 Markdown 格式引用：
‌
1
...
本地部署
想在本机预览或修改 Wiki，需要先安装依赖（需已安装 Python）：
‌
1
